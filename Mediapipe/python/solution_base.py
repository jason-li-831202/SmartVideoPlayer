...
class SolutionBase:
  ...
  def __init__(
    ...
    ## root_path = os.sep.join(os.path.abspath(os.getcwd()).split(os.sep))
    ...
    if getattr(sys, 'frozen', False):
        ...
        root_path = Path()
        root_path_tuple = Path(__file__).resolve().parts[:-3]
        for path in root_path_tuple :
          root_path = root_path.joinpath(path)

    # model_path = os.path.join(root_path, binary_graph_path)
    # if not os.path.isfile(model_path):
    #     model_path = os.path.expanduser(os.path.join(root_path, "Mediapipe/",binary_graph_path))  
    binary_graph_path = Path(binary_graph_path)
    model_path = root_path.joinpath(binary_graph_path)
    if not Path.is_file(model_path):
      model_path = str(Path.expanduser(Path.joinpath(root_path, Path("Mediapipe/"), binary_graph_path)))
    else :
      model_path = str(model_path)

    resource_util.set_resource_dir(str(root_path))
    validated_graph = validated_graph_config.ValidatedGraphConfig()
    if binary_graph_path:
      validated_graph.initialize(
          binary_graph_path=model_path)
    else:
