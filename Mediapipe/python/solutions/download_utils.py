...
def download_oss_model(model_path: str):
    ...
    #   return
    mp_root_path = Path()
    mp_root_path_tuple = Path(__file__).resolve().parts[:-4]
    for path in mp_root_path_tuple :
      mp_root_path = mp_root_path.joinpath(path)
    model_abspath = Path.joinpath(mp_root_path, Path(model_path))
    if Path.is_file(model_abspath):
      return
